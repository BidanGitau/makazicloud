...
import { X, Mail, FileText, CheckCircle, AlertCircle, Loader } from "lucide-react";
import { sendInvoiceEmails, sendStatementEmails } from "@/app/_lib/sendEmail";
...
const CONFIG = {
  invoice: {
    ...
    pdfNote: "The invoice PDF will be auto-generated and attached.",
  },
  statement: {
    ...
    accentColor: "blue",
    defaultMessage:
      "Please find attached your account statement showing all charges and payments to date. Please review and contact us if you have any questions.",
    pdfNote: "A full statement with running balance will be auto-generated and attached.",
  },
};
...
export default function SendDocumentModal({ isOpen, onClose, docType = "invoice", tenants = [] }) {
  ...
  const config = CONFIG[docType] || CONFIG.invoice;
  const currentMessage = message || config.defaultMessage;
  ...
  const accentBg = config.accentColor === "blue" ? "bg-blue-600" : "bg-blue-600";
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-gray-900">
              {config.title}{tenants.length > 1 ? ` (${tenants.length})` : ""}
            </h2>
          </div>
        ...
        <div className="px-5 py-4 space-y-4">
            ...
            <>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">
                  Message{" "}
                  <span className="text-gray-400 font-normal">(included in email body)</span>
                </label>
                <textarea
              ...
              <p className="text-xs text-gray-400">{config.pdfNote}</p>
            </>
          )}
          ...
          {status === "sending" && (
            <div className="flex flex-col items-center gap-3 py-6 text-gray-600">
              ...
              <p className="text-sm">
                Sending {tenants.length > 1 ? `${tenants.length} emails` : "email"}…
              </p>
            </div>
          )}
        ...
        <div className="flex justify-end gap-3 px-5 py-4 border-t border-gray-200">
          ...
          {status === "idle" && (
            <button
              ...
              className={`px-4 py-2 text-sm rounded-lg text-white flex items-center gap-2 disabled:opacity-50 ${accentBg} hover:opacity-90`}
            >
