...
export default function UtilityPage() {
  ...
  const { propertyBlocks: filterBlocks, propertyUnits: filterUnits, allUnits, isLoading: isLoadingFormData } =
  ...
  const filteredBills = useMemo(() => {
    return bills.filter((b) => {
      ...
      if (filters.month && !b.billing_month?.startsWith(filters.month)) return false;
      return true;
    });
  }, [bills, filters]);

  const stats = useMemo(() => {
    const total = filteredBills.reduce((s, b) => s + Number(b.total_amount || 0), 0);
    const paid = filteredBills.reduce((s, b) => s + Number(b.paid_amount || 0), 0);
    const pending = filteredBills.filter((b) => b.status !== "paid").length;
    return { total, paid, pending, count: filteredBills.length };
  }, [filteredBills]);

  const handleMarkPaid = useCallback(async (bill) => {
    try {
  ...
  return (
    <div className="space-y-5 p-3 sm:p-6">
      ...
      <div className="grid grid-cols-2 gap-px border border-stone-200 bg-stone-200 md:grid-cols-4">
        ...
        <StatCard label="Total Paid"    value={`KSh ${stats.paid.toLocaleString("en-KE")}`}  accent="text-green-700" />
      ...
      <div className="border border-stone-200 bg-white p-4">
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          <select
            ...
            onChange={(e) => setFilters({ ...FILTER_INIT, property: e.target.value })}
          ...
          <select
            value={filters.block}
            onChange={(e) => setFilters((f) => ({ ...f, block: e.target.value, unit: "" }))}
