...
export default function MaintenancePage() {
  ...
  const { properties, isLoading: isLoadingFormData } = useFormData({ includeUnits: false });
  ...
  const filteredRequests = useMemo(() =>
    requests.filter((r) => {
      if (filters.property && r.property_id !== filters.property) return false;
  ...
  const stats = useMemo(() => {
    ...
    const totalCost  = filteredRequests.reduce(
      (s, r) => s + Number(r.actual_cost ?? r.estimated_cost ?? 0), 0,
    );
    const totalAdvances = filteredAdvances.reduce(
      (s, a) => s + Number(a.amount || 0), 0,
    );
    return { total: filteredRequests.length, pending, inProgress, completed, totalCost, totalAdvances };
  }, [filteredRequests, filteredAdvances]);

  const closeModal = useCallback(() => { setActiveModal(null); setEditTarget(null); }, []);
  ...
  const requestColumns = useMemo(() =>
    buildMaintenanceColumns({
      onEdit:         canEdit ? (row) => { setEditTarget(row); setActiveModal("edit_request"); } : null,
  ...
  const advanceColumns = useMemo(() =>
    buildAdvanceColumns({
      onEdit:   canEdit ? (row) => { setEditTarget(row); setActiveModal("edit_advance"); } : null,
  ...
  return (
    <div className="space-y-5 p-3 sm:p-6">
      ...
      <div className="grid grid-cols-2 gap-px border border-stone-200 bg-stone-200 md:grid-cols-3 lg:grid-cols-6">
        ...
        <StatCard label="In Progress"    value={stats.inProgress}                     accent="text-blue-700" />
        <StatCard label="Completed"      value={stats.completed}                      accent="text-green-700" />
        <StatCard label="Total Cost"     value={formatCurrency(stats.totalCost)}      accent="text-red-600" />
      ...
      <div>
        {tab === "requests" ? (
          <DataTable
            ...
            noDataComponent={
              <div className="py-10 text-center text-gray-500 text-sm">
                No maintenance requests found{hasFilters ? " for the selected filters" : ""}.
              </div>
            }
            responsive
            striped
          ...
          <DataTable
            ...
            noDataComponent={
              <div className="py-10 text-center text-gray-500 text-sm">
                No owner advances found{filters.property ? " for this property" : ""}.
              </div>
            }
            responsive
            striped
      ...
      <ModalSlider
        ...
        onClose={closeModal}
        title={activeModal === "edit_request" ? "Edit Maintenance Request" : "Add Maintenance Request"}
      >
        <MaintenanceForm
          ...
          onSuccess={() => {
            closeModal();
            showToast.success(editTarget ? "Request updated." : "Request added.");
            fetchAll();
          }}
        />
      </ModalSlider>
      ...
      <ModalSlider
        ...
        onClose={closeModal}
        title={activeModal === "edit_advance" ? "Edit Owner Advance" : "Add Owner Advance"}
      >
        <AdvanceForm
